...
/* Set up stack pointers for IRQ and SVC processor modes */
                MOV      R0, #0b10010        // interrupts masked, MODE = IRQ
				MSR      CPSR, R0            // change to IRQ mode
				LDR      SP, =0x20000        // set IRQ stack pointer
				
				MOV		 R0, #0b10011        // interrupts masked, MODE = SVC
				MSR      CPSR, R0            // change to supervisor mode
				LDR      SP, =0x3FFFFFFC     // set SVC stack

                BL       CONFIG_GIC          // configure the ARM generic interrupt controller
											 
/* Configure the KEY pushbuttons port to generate interrupts */
                LDR      R1, =0xFF200058     // pushbutton KEY base address
				MOV      R2, #0xF            // set interrupt mask bits
				STR      R2, [R1]            // interrupt mask register
	
/* Configure the Interval Timer to create interrupts at 0.25 second intervals */
CONFIG_TIMER:                             
				LDR 	 R0, =0xFF202008
				LDR 	 R1, =0x7840
				STR  	 R1, [R0]            // store the lower half of the word
				
				LDR 	 R0, =0xFF20200C
				LDR 	 R1, =0x17D
				STR 	 R1, [R0]            // store the upper half of the word
				
				LDR 	 R0, =0xFF202004
				MOV 	 R1, #0b101          // start the interval timer
				STR 	 R1, [R0]  
...
KEY_ISR:        
				PUSH     {R0-R3, LR}
				LDR      R3, =0xFF200020             // base HEX0-HEX3 address
				LDR      R1, [R0, #0xC]
				STR      R1, [R0, #0xC]
				
				LDR      R0, =0xFF200050             // load KEY address
				LDR      R1, [R0, #0xC]              // reset the edge capture and advance to edge
				STR      R1, [R0, #0xC]
...
END_KEY: 		
				STR      R2, [R3]
				POP      {R0-R3, LR}          // pop all the registers
				MOV      PC, LR
				
TIMER_ISR: 		PUSH     {R0-R4, LR}
				LDR      R0, COUNT
				LDR 	 R3, =COUNT
				LDR      R1, RUN
				ADD  	 R2, R0, R1
